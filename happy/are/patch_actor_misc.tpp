  ...
  FOR (index = 0 ; index < actor_num ; index = index + 1) BEGIN
    ...
    PATCH_IF NOT (("%resref%" STR_EQ "NONE") OR ("%resref%" STR_EQ "")) BEGIN
      PATCH_IF (("%flags%" BAND 0b00000001) = 0b00000000) BEGIN
        PATCH_PRINT "HI9: %SOURCE_FILE% - Actor #%index% (%resref%.CRE) exists and should not be attached to the ARE..."
    ...
    PATCH_IF ("%removal_timer%" = 0) BEGIN
      PATCH_PRINT "HW2: %SOURCE_FILE% - Actor #%index% set to disappear after %removal_timer% seconds! Setting to -1..."
      
      WRITE_LONG ("%actor_off%" + 0x38 + (0x110 * "%index%")) 0xffffffff
    END ELSE
    PATCH_IF ("%removal_timer%" != 0xffffffff) BEGIN
      PATCH_PRINT "HW3: %SOURCE_FILE% - Actor #%index% set to disappear after %removal_timer% seconds!"
//        WRITE_LONG ("%actor_off%" + 0x38 + (0x110 * "%index%")) 0xffffffff
    END
  END
END
