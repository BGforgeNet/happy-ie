PATCH_IF (SOURCE_SIZE > 0x71) BEGIN
  ...
  PATCH_IF ((abil_off > fx_off) AND (abil_num > 1)) BEGIN
    ...
    offset = abil_off
    value  = 0x28 * abil_num
    PATCH_PRINT "HI54: %SOURCE_FILE% - Restructuring - Unused Ability Structure found! Trimming %value% bytes from offset %offset%..."
  ...
  PATCH_IF ((abil_off + (0x28 * abil_num)) < fx_off) BEGIN
    ...
    PATCH_PRINT "HI55: %SOURCE_FILE% - Restructuring - Unused Inter-structural Bytes found! Trimming %value% bytes from offset %offset%..."
    DELETE_BYTES offset value
    SOURCE_SIZE = SOURCE_SIZE - value
    fx_off = offset
    WRITE_LONG 0x6a %fx_off%
  ...
  PATCH_IF (SOURCE_SIZE > (0x72 + (0x28 * abil_num) + (0x30 * fx_num))) BEGIN
    ...
    PATCH_PRINT "HI56: %SOURCE_FILE% - Restructuring - Unused Trailing Bytes found! Trimming %value% bytes from offset %offset%..."
    DELETE_BYTES offset value
    SOURCE_SIZE = SOURCE_SIZE - value
  END
END
