...
PHP_EACH h_array AS i1 => h_off BEGIN
  ...
  PHP_EACH fx_array AS i2 => fx_off BEGIN
    ...
    PATCH_IF (timing = 1 OR timing = 9) AND duration !=0 BEGIN
      PATCH_PRINT ~%SOURCE_FILE% issue: effect %i1%/%i2% has useless duration of %duration%. Setting to 0.~
      WRITE_LONG (fx_off + 0xe) 0
    END
    PATCH_IF (timing = 0) AND duration = 0 BEGIN
      PATCH_PRINT ~%SOURCE_FILE% issue: effect %i1%/%i2% has no duration set. Setting to timing to 1/instant.~
