  ...
  FOR (index = 0; index < abil_num; index = index + 1) BEGIN
    ...
    PATCH_IF NOT (FILE_EXISTS_IN_GAME ~%abil_icon%.bam~) AND ("%abil_type%" > 0) BEGIN
      PATCH_IF ("%abil_icon%" STR_EQ "none") OR ("%abil_icon%" STR_EQ "") BEGIN
        PATCH_PRINT "HW88: %SOURCE_FILE% - Ability #%index% has no icon assigned! Assigning Inventory Icon (%inv_icon%.BAM)..."
      END ELSE
      BEGIN
        PATCH_PRINT "HW89: %SOURCE_FILE% - Ability #%index% has a non-existent icon (%abil_icon%.BAM)! Assigning Inventory Icon (%inv_icon%.BAM)..."
      END
