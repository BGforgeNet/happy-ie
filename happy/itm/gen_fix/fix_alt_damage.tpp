...
PHP_EACH head_array AS i => head_off BEGIN
  ...
  PATCH_IF alt_dice_side != 0 OR alt_dice_num != 0 OR alt_dam_bonus != 0 BEGIN
    PATCH_PRINT ~HI36: %SOURCE_FILE% - header %i% has alternative damage set to %alt_dice_num%d%alt_dice_side%+%alt_dam_bonus%. Setting to 0.~
    WRITE_BYTE (head_off + 0x3) 0
    WRITE_BYTE (head_off + 0x11) 0
    WRITE_BYTE (head_off + 0x13) 0
  END
END
