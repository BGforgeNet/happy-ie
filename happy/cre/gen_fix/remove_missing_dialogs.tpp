...
PATCH_IF NOT (("%dialog%" STR_EQ "NONE") OR ("%dialog%" STR_EQ "")) BEGIN
  ...
  PATCH_IF ("%exists%" = 0) AND ("%dlgexists%" = 1) BEGIN
    PATCH_PRINT "HW19: %SOURCE_FILE% - Non-existent dialogue (%dialog%.DLG) changed into %SOURCE_RES%.DLG..."
    WRITE_ASCIIE 0x2cc ~%SOURCE_RES%~ #8
  END ELSE
  PATCH_IF ("%exists%" = 0) BEGIN
    PATCH_PRINT "HI20: %SOURCE_FILE% - Non-existent dialogue (%dialog%.DLG) nulled..."
    WRITE_ASCII 0x2cc ~~ #8
  END
END
